...
static void gc_mark(global_State *g, GCobj *o)
{
  ...
  if (LJ_UNLIKELY(gct == ~LJ_TUDATA)) {
    ...
    setgcrefr(o->gch.gclist, g->gc.gray[o->gch.bucket]);
    setgcref(g->gc.gray[o->gch.bucket], o);
  }
}
...
static GCRef *gc_sweep(global_State *g, uint8_t b, GCRef *p, uint32_t lim)
{
  ...
  while ((o = gcref(*p)) != NULL && lim-- > 0) {
    ...
    if (((o->gch.marked ^ LJ_GC_WHITES) & ow)) {  /* Black or current white? */
      ...
      if (b == 0xFF) { /* Might need to sweep all buckets depending on what we are dealing with. */
        if (o == gcref(g->gc.root[o->gch.bucket]))
	        setgcrefr(g->gc.root[o->gch.bucket], o->gch.nextgc);  /* Adjust list anchor. */
      } else {
...
void lj_gc_freeall(global_State *g)
{
  ...
  for (i = 0; i <= strmask; i++)  /* Free all string hash chains. */
    gc_fullsweep(g, 0xFF, &g->strhash[i]);
}
...
static size_t gc_onestep(lua_State *L, uint8_t b)
{
  ...
  case GCSsweepstring: {
    ...
    gc_fullsweep(g, 0xFF, &g->strhash[g->gc.sweepstr++]);  /* Sweep one chain. */
...
void LJ_FASTCALL lj_gc_step_fixtop_all(lua_State *L)
{
  if (curr_funcisL(L)) L->top = curr_topL(L);
  lj_gc_step_all(L);
}
...
int LJ_FASTCALL lj_gc_step_jit_bucket(global_State *g, uint8_t b, MSize steps)
{
...
{
  return lj_gc_step_jit_bucket(g, 0, steps);
}
#endif
...
void lj_gc_fullgc(lua_State *L, uint8_t b)
{
  ...
  if (g->gc.state <= GCSatomic) {  /* Caught somewhere in the middle. */
    setmref(g->gc.sweep[b], &g->gc.root[b]);  /* Sweep everything (preserving it). */
